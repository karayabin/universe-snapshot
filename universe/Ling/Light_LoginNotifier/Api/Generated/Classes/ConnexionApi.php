...
namespace Ling\Light_LoginNotifier\Api\Generated\Classes;
...
use Ling\Light_LoginNotifier\Api\Custom\Classes\CustomLightLoginNotifierBaseApi;
use Ling\Light_LoginNotifier\Api\Generated\Interfaces\ConnexionApiInterface;
 ...
 */
class ConnexionApi extends CustomLightLoginNotifierBaseApi implements ConnexionApiInterface
{
    ...
    { 
        ...
        try {
            ...
            $lastInsertId = $this->pdoWrapper->insert($this->table, $connexion);
            if (false === $returnRic) {
                return $lastInsertId;
            }
            $ric = [
        ...
        if (null !== $errorInfo) {
            if ('23000' === $errorInfo[0]) {
                ...
                $allMarkers = [];
                SimplePdoWrapper::addWhereSubStmt($query, $allMarkers, $connexion);
    ...
    public function insertConnexions(array $connexions, bool $ignoreDuplicate = true, bool $returnRic = false)
    {
        ...
        foreach ($connexions as $connexion) {
            $res = $this->insertConnexion($connexion, $ignoreDuplicate, $returnRic);
            if (false === $res) {
                return false;
            }
            $ret[] = $res;
    ...
    {
        $this->pdoWrapper->update($this->table, $connexion, array_merge([
            "id" => $id,
    ...
    {
        $this->pdoWrapper->update($this->table, $connexion, $where, $markers);
    }
