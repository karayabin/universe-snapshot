...
/**
 * The CartItemApi class.
 */
class CartItemApi extends CustomLightKitStoreBaseApi implements CartItemApiInterface
{


    /**
     * Builds the CartItemApi instance.
     */
    public function __construct()
    {
        parent::__construct();
        $this->table = "lks_cart_item";
    }
    ...
    { 
        ...
        $cartItem = array_replace($this->getDefaultValues(), $cartItem);

        try {

            $lastInsertId = $this->pdoWrapper->insert($this->table, $cartItem);
            if (false === $returnRic) {
                return $lastInsertId;
            }
            $ric = [
        ...
        if (null !== $errorInfo) {
            if ('23000' === $errorInfo[0]) {
                ...
                $allMarkers = [];
                SimplePdoWrapper::addWhereSubStmt($query, $allMarkers, $cartItem);
    ...
    public function insertCartItems(array $cartItems, bool $ignoreDuplicate = true, bool $returnRic = false)
    {
        ...
        foreach ($cartItems as $cartItem) {
            $res = $this->insertCartItem($cartItem, $ignoreDuplicate, $returnRic);
            if (false === $res) {
                return false;
            }
            $ret[] = $res;
    ...
    {
        $this->pdoWrapper->update($this->table, $cartItem, array_merge([
            "id" => $id,
    ...
    {
        $this->pdoWrapper->update($this->table, $cartItem, $where, $markers);
    }
