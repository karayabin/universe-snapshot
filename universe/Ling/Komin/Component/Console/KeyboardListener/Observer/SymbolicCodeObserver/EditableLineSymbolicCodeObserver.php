...
use Ling\Komin\Component\Console\KeyboardListener\Observer\SymbolicCodeObserver\EditableLineSymbolicCodeObserver\Behaviour;
 ...
 *
 */
class EditableLineSymbolicCodeObserver implements SymbolicCodeObserverInterface
{
     ...
     *              - 1: mixed  callback ( keyCode, editableLine, keyboardListener, fromRead, preBehaviour )
     *
     ...
     *                              - preBehaviour, a PreBehaviour object
     *
     ...
     *              - 0: keyCodes
     *                      same as preEvent
     *              - 1: mixed  callback ( keyCode, editableLine, keyboardListener, fromRead, behaviour )
     *                      same as preEvent, except for the last which is a Behaviour instead of a PreBehaviour
     *
     */
    private $postEvents;
    /**
     * Like postEvents, but just for the burst event
     */
    private $burstEvents;
    ...
    public function notify($code, $string, KeyboardListenerInterface $keyboardListener, $fromRead = true)
    {
        ...
        /**
         * Taking care of the initial burst signal upfront, so that it never reaches pre/post events.
         */
        if ('' === $code) {
            $behaviour = new Behaviour();
            foreach ($this->burstEvents as $event) {
                list($keyCodes, $callback) = $event;
                if (null === $keyCodes || in_array($code, $keyCodes, true)) {
                    call_user_func($callback, $code, $this->editableLine, $keyboardListener, $fromRead, $behaviour);
                    if (true === $behaviour->isPropagationStopped()) {
                        break;
                    }
                }
            }
            $this->editableLine->refreshLine(); // allow for prompt to show up
            return;
        }

        $s = '';
        $preBehaviour = new PreBehaviour();
        foreach ($this->preEvents as $event) {
            ...
            if (null === $keyCodes || in_array($code, $keyCodes, true)) {
                 ...
                 *
                 */
                $s .= call_user_func_array($callback, [$code, $this->editableLine, $keyboardListener, $fromRead, $preBehaviour]);
                if (true === $preBehaviour->isPropagationStopped()) {
                    break;
                }
            }
        }
         ...
         * Default behaviour
         */
        if (false === $preBehaviour->isDefaultBehaviourBypassed()) {
        ...
        if (false === $preBehaviour->isRefreshPrevented()) {
        ...
        foreach ($this->postEvents as $event) {
            list($keyCodes, $callback) = $event;
            if (null === $keyCodes || in_array($code, $keyCodes, true)) {
                $s .= call_user_func($callback, $code, $this->editableLine, $keyboardListener, $fromRead, $behaviour);
                if (true === $behaviour->isPropagationStopped()) {
                    break;
                }
            }
...
//        }
    ...
    // 
    //------------------------------------------------------------------------------/
    public function setBurstEvent($callback, $keyCodes = 'return', $index = null)
    {
        $this->setEvent($this->burstEvents, $callback, $keyCodes, $index);
    }
