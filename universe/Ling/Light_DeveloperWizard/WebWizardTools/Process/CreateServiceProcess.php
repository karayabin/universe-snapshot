...
class CreateServiceProcess extends LightDeveloperWizardBaseProcess implements LightServiceContainerAwareInterface
{
    ...
    {
        parent::__construct();
        $this->setName("create-service-class");
        $this->setLabel("Create/update the service class and config of a planet.");
        $this->container = null;
        $this->util = null;
    }
    ...
    protected function doExecute(array $options = [])
    {
        ...
        $hasClassFile = $this->util->hasServiceClassFile();


        if (true === $hasClassFile) {
            $planetIdentifier = $this->util->getPlanetIdentifier();

            $this->infoMessage("The planet $planetIdentifier already has a service file. Aborting.");
