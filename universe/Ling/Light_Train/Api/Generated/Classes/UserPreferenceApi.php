...
namespace Ling\Light_Train\Api\Generated\Classes;
...
use Ling\Light_Train\Api\Custom\Classes\CustomLightTrainBaseApi;
 ...
 */
class UserPreferenceApi extends CustomLightTrainBaseApi implements UserPreferenceApiInterface
{
    ...
    {
        parent::__construct();
        $this->table = "lup_user_preference";
    }
    ...
    { 
        ...
        try {
            ...
            $lastInsertId = $this->pdoWrapper->insert($this->table, $userPreference);
            if (false === $returnRic) {
                return $lastInsertId;
            }
            $ric = [
        ...
        if (null !== $errorInfo) {
            if ('23000' === $errorInfo[0]) {
                ...
                $allMarkers = [];
                SimplePdoWrapper::addWhereSubStmt($query, $allMarkers, $userPreference);
    ...
    public function insertUserPreferences(array $userPreferences, bool $ignoreDuplicate = true, bool $returnRic = false)
    {
        ...
        foreach ($userPreferences as $userPreference) {
            $res = $this->insertUserPreference($userPreference, $ignoreDuplicate, $returnRic);
            if (false === $res) {
                return false;
            }
            $ret[] = $res;
     ...
     * @implementation
     */
    public function updateUserPreferenceById(int $id, array $userPreference)
    { 
        $this->pdoWrapper->update($this->table, $userPreference, [
            "id" => $id,
