 ...
 *
 */
class ChownCommand extends BaseCommand
{
    ...
    protected function doExecute(array $params)
    {
        ...
        foreach ($params as $rule) {
            ...
            if (count($p) > 1) {
                list($target, $perms) = $p;
                $cmd = 'chown -R ' . $perms . ' "' . trim($target, '"') . '" 2>&1';
                echo $cmd . PHP_EOL;
                ob_start();
                passthru($cmd);
                $r = ob_get_clean();
                ...
                $this->log('invalidFormat.colon', sprintf("This rule does not contain a colon to separate the target from the ownership identifier to apply to it: %s", $rule));
                $ret = false;
            }
        }
        return $ret;
